...
async function getCampaignRow(campaignId: string) {
  ...
  if (row.mechanic !== 'buy-x-get-y') throw new Error('NOT_BUY_X_GET_Y_CAMPAIGN')
  return row
}

export async function getBuyXGetYState(campaignId: string, customerId: string) {
  const row = await getCampaignRow(campaignId)
  const config = parseBuyXGetYConfig((row.config_json as string) ?? null)
  if (!config) throw new Error('INVALID_BUY_X_GET_Y_CONFIG')
  ...
  const existing = await db.execute({
    sql: `SELECT * FROM customer_rewards
          WHERE campaign_id = ? AND customer_id = ? AND source_type = 'buy_x_get_y'
          LIMIT 1`,
  ...
  const claimedCount = await db.execute({
    sql: `SELECT COUNT(*) AS c FROM customer_rewards
          WHERE campaign_id = ? AND source_type = 'buy_x_get_y'`,
    args: [campaignId],
  })

  const userCap = Number(row.user_cap ?? 200)
  ...
  return {
    ...
    active,
    canClaim: active && !reward && claimed < userCap,
    hasClaimed: Boolean(reward),
    claimedCount: claimed,
    userCap,
    spotsRemaining: Math.max(0, userCap - claimed),
    offerSentence: formatBuyXGetYSentence(config),
    rewardLabel,
    rewardDescription: formatBuyXGetYDescription(config),
...
export async function claimBuyXGetYReward(
  campaignId: string,
  customerId: string,
  ...
  const userCap = Number(row.user_cap ?? 200)
  if (Number(claimedCount.rows[0]?.c ?? 0) >= userCap) {
  ...
  await db.batch([
    ...
    },
    {
      sql: `INSERT INTO customer_rewards
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'earned', datetime('now'), ?, 'buy_x_get_y', ?)`,
      args: [
        rewardId,
        customerId,
        campaignId,
  ...
  return {
    rewardId,
    reward: rewardLabel,
    description: formatBuyXGetYDescription(config),
    offerSentence: formatBuyXGetYSentence(config),
    code: redemptionCode,
