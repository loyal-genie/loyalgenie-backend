...
async function getCampaignRow(campaignId: string) {
  ...
  if (row.mechanic !== 'friend') throw new Error('NOT_FRIEND_CAMPAIGN')
  return row
}

export async function getFriendState(campaignId: string, customerId: string) {
  const row = await getCampaignRow(campaignId)
  const config = parseFriendConfig((row.config_json as string) ?? null)
  if (!config) throw new Error('INVALID_FRIEND_CONFIG')
  ...
  const existing = await db.execute({
    sql: `SELECT * FROM customer_rewards
          WHERE campaign_id = ? AND customer_id = ? AND source_type = 'friend'
          LIMIT 1`,
  ...
  const claimedCount = await db.execute({
    sql: `SELECT COUNT(*) AS c FROM customer_rewards
          WHERE campaign_id = ? AND source_type = 'friend'`,
    args: [campaignId],
  })
  ...
  return {
    ...
    offerSentence: formatFriendSentence(config),
    rewardLabel,
    rewardDescription: formatFriendDescription(config),
...
export async function claimFriendReward(
  campaignId: string,
  customerId: string,
  ...
  await db.batch([
    ...
    },
    {
      sql: `INSERT INTO customer_rewards
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'earned', datetime('now'), ?, 'friend', ?)`,
      args: [
        rewardId,
        customerId,
        campaignId,
  ...
  return {
    rewardId,
    reward: rewardLabel,
    description: formatFriendDescription(config),
    offerSentence: formatFriendSentence(config),
    code: redemptionCode,
